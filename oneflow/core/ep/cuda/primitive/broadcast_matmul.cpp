...
Optional<hipblasDatatype_t> OptCudaDataType(DataType data_type) {
  switch (data_type) {
    ...
    case kComplex64: return HIPBLAS_C_32F;
    case kComplex128: return HIPBLAS_C_64F;
// #if CUDA_VERSION >= 11000
//     case kBFloat16: return CUDA_R_16BF;
// #endif  // CUDA_VERSION >= 11000
    default: return NullOpt;
...
void LaunchBroadcastMatmul(Stream* stream, DataType data_type, BlasTransposeType transpose_a,
  ...
  const auto GetCublasOperation = [](BlasTransposeType transpose_type, DataType data_type) {
    if (transpose_type == BlasTransposeType::N) {
      ...
      return DType(data_type).is_complex() ? HIPBLAS_OP_C : HIPBLAS_OP_T;
    } else {
      UNIMPLEMENTED();
      return HIPBLAS_OP_N;
    }
  };
  const hipblasOperation_t cublas_trans_a = GetCublasOperation(transpose_b, data_type);
  const hipblasOperation_t cublas_trans_b = GetCublasOperation(transpose_a, data_type);
  const int cublas_m = n;
  const int cublas_n = m;
  const int cublas_k = k;
  int cublas_lda = 0;
