...
class CuFFTHandle {
 ...
 public:
  CuFFTHandle() { OF_CUFFT_CHECK(hipfftCreate(&handle)); }

  hipfftHandle& get() { return handle; }
  const hipfftHandle& get() const { return handle; }

  ~CuFFTHandle() { hipfftDestroy(handle); }
};
...
class CuFFTConfig {
 public:
  ...
  explicit CuFFTConfig(CuFFTParams& params) {  // NOLINT
    ...
    OF_CUFFT_CHECK(hipfftSetAutoAllocation(plan_handle_.get(), 0));
 ...
 private:
  void infer_cufft_type_(CUFFT_EXCUTETYPE excute_type, DataType real_data_type) {
    if (real_data_type == kFloat) {
      switch (excute_type) {
        case CUFFT_EXCUTETYPE::R2C: 
          data_type_desc_.executiontype = HIPFFT_R2C;
          break;
        case CUFFT_EXCUTETYPE::C2R: 
          data_type_desc_.executiontype = HIPFFT_C2R;
          break;
        case CUFFT_EXCUTETYPE::C2C: 
          data_type_desc_.executiontype = HIPFFT_C2C;
          break;
        default: 
          CHECK_OR_THROW(false) << "hipFFT doesn't support this execution type ";
          break;
      }
    } else if (real_data_type == kDouble) {
      switch (excute_type) {
        case CUFFT_EXCUTETYPE::R2C: 
          data_type_desc_.executiontype = HIPFFT_D2Z;
          break;
        case CUFFT_EXCUTETYPE::C2R: 
          data_type_desc_.executiontype = HIPFFT_Z2D;
          break;
        case CUFFT_EXCUTETYPE::C2C: 
          data_type_desc_.executiontype = HIPFFT_Z2Z;
          break;
        default: 
          CHECK_OR_THROW(false) << "hipFFT doesn't support this execution type ";
          break;
