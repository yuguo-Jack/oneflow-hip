...
class StftGpuKernel final : public user_op::OpKernel {
 ...
 private:
  ...
  void Compute(user_op::KernelComputeContext* ctx) const override {
    ...
    OF_CUFFT_CHECK(GPUFFT(SetStream)(plan, ctx->stream()->As<ep::CudaStream>()->cuda_stream()));
    void* workspace{};
    OF_CUDA_CHECK(GPU(Malloc)(&workspace, config.workspace_size()));
    OF_CUFFT_CHECK(GPUFFT(SetWorkArea)(plan, workspace));
    ...
    OF_CUDA_CHECK(GPU(Free)(workspace));
...
class FftC2CKernelUtil<DeviceType::kCUDA, T, FCT_TYPE> {
  static void FftC2CForward(ep::Stream* stream, const T* data_in, T* data_out,
    ...
    CuFFTConfig config(params);
    auto& plan = config.plan();
    OF_CUFFT_CHECK(GPUFFT(SetStream)(plan, stream->As<ep::CudaStream>()->cuda_stream()));
    void* workspace{};
    OF_CUDA_CHECK(GPU(Malloc)(&workspace, config.workspace_size()));
    OF_CUFFT_CHECK(GPUFFT(SetWorkArea)(plan, workspace));
